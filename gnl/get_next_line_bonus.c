...
/*   Updated: 2025/01/08 14:59:56 by junhykim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_safe	buffer[ULIMIT];
	char			*go;

	if (BUFFER_SIZE <= 0 || ULIMIT <= 2)
		return (0);
	go = 0;
	if (!buffer[fd].type)
		if (!buffer_read(fd, &buffer[fd]))
			return (NULL);
	while (!gnl_bufchr(buffer[fd], '\n'))
	{
		go = buffer_plant(go, buffer[fd]);
		buffer[fd] = buffer_clear(buffer[fd]);
		if (!buffer_read(fd, &buffer[fd]))
		{
			if (go)
				return (go);
	...
	go = buffer_plant(go, buffer[fd]);
	buffer[fd] = buffer_set(buffer[fd]);
	return (go);
}
